...
mod tests {
    ...
    #[test]
    fn retained_runtime_visitor_rejects_calls_after_traversal() {
        let mut retained = None;
        structural_walk(
            &1i64,
            |_value: i64| {
                let handle = active_structural_visitor().unwrap();
                // Own a reference before the traversal releases its visitor.
                retained = Some(Any::from(unsafe { borrowed_visitor_view(handle) }));
                WalkResult::Advance
            },
            WalkOrder::PreOrder,
        )
        ...
        let error = Function::get_global("ffi.StructuralVisitorVisit")
            .unwrap()
            .call_tuple((retained.unwrap(), 1i64))
            .err()
    ...
    #[test]
    fn runtime_visitor_rejects_foreign_thread_calls() {
        structural_walk(
            &1i64,
            |_value: i64| {
                let address = active_structural_visitor().unwrap() as usize;
                // The traversal keeps the visitor alive until this worker joins.
                // The worker only borrows it and never touches its reference count.
                let message = std::thread::spawn(move || {
                    let visitor =
                        unsafe { borrowed_visitor_view(address as StructuralVisitorHandle) };
                    Function::get_global("ffi.StructuralVisitorVisit")
                        .unwrap()
                        .call_packed(&[visitor, AnyView::from(&1i64)])
                        .err()
    ...
    #[test]
    fn function_visit_hook_dispatch_does_not_require_registration() {
        ...
        impl StructuralVisitor for FunctionVisitor {
            fn visit(
                &mut self,
                value: &VisitValue,
                kind: DefRegionKind,
                ...
                if let Some(integer) = value.cast::<i64>() {
                    self.seen.push((integer, kind));
                    return Ok(self
                        .interrupt
                        .then(|| VisitInterrupt::with(crate::String::from("stop"))));
                }
                let handle = active_structural_visitor()?;
                let attr = raw_of(AnyView::from(&self.hook));
                let context = std::ptr::from_mut(self).cast::<c_void>();
                // Exercise the Function ABI directly with a local hook value.
                // No object type or shared type attribute needs to be registered.
                finish(with_current_visitor_context(handle, context, || {
                    call_structural_visit_hook(handle, value.raw(), DefRegionKind::Recursive, attr)
                }))
            }
        }

        let hook = Function::from_packed(|args| {
            assert_eq!(args.len(), 2);
            let kind = Function::get_global("ffi.StructuralVisitorDefRegionKind")?
                .call_packed(&[args[0]])?;
            assert_eq!(i64::try_from(kind)?, DefRegionKind::Recursive as i64);
            let array = Array::<i64>::try_from(args[1])?;
            let selected = array.get(0).unwrap();
            Function::get_global("ffi.StructuralVisitorVisit")?
                .call_packed(&[args[0], AnyView::from(&selected)])
        });
