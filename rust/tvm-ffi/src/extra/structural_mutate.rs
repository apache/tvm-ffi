...
pub use super::StructuralValue;

/// Compatibility name for [`StructuralValue`].
pub use super::StructuralValue as MapValue;
...
pub struct MutateValue<'a, T = Any> {
    ...
    mode: InplaceMode,
    _scope: PhantomData<&'a StructuralValue>,
    _type: PhantomData<fn() -> T>,
    _not_send_sync: PhantomData<Rc<()>>,
}

impl<'a> MutateValue<'a> {
    fn new(value: &'a StructuralValue, mode: InplaceMode) -> Self {
        Self {
            value: StructuralValue::from_raw(value.raw()),
            mode,
            _scope: PhantomData,
            _type: PhantomData,
    ...
    pub fn borrowed(value: &'a StructuralValue) -> Self {
...
impl<'a, T> MutateValue<'a, T> {
    ...
    pub fn as_value(&self) -> &StructuralValue {
        &self.value
    }
...
pub trait MutateContextDriver<State> {
    ...
    fn var_remap_get(&mut self, var: &StructuralValue) -> Result<Option<Any>>;
    fn var_remap_set(&mut self, var: &StructuralValue, mutated_value: &Any) -> Result<()>;
}
    ...
    Driver: MutateContextDriver<State> + ?Sized,
{
    ...
    /// Look up an invocation-local identity substitution.
    #[inline(always)]
    pub fn var_remap_get(&mut self, var: &StructuralValue) -> Result<Option<Any>> {
        self.driver.var_remap_get(var)
    }

    /// Store an invocation-local identity substitution.
    #[inline(always)]
    pub fn var_remap_set(&mut self, var: &StructuralValue, mutated_value: &Any) -> Result<()> {
...
#[diagnostic::on_unimplemented(
    ...
    note = "accepted mutators: `&mut U` where `U: StructuralMutator`; a generated `MutateDispatch`; an `Fn` callback over an FFI value type `T`, `&N` of an object node type, or `&StructuralValue`, or a consuming `MutateValue<T>`, followed by `&mut CallbackMutator<State>`; or a tuple of up to 12 such callbacks (tuples may nest)",
...
mod mutate_sealed {
    use super::{IntoMutateResult, MutateContext, MutateValue, ObjectCore, StructuralValue};
...
/// object node, or `&StructuralValue`, optionally followed by [`DefRegionKind`].
...
mod sealed_map {
    use super::{DefRegionKind, IntoMapResult, MapDispatch, ObjectCore, StructuralValue};
    ...
    where
        F: for<'a> FnMut(&'a StructuralValue) -> O,
        O: IntoMapResult,
    {
    }
    ...
    where
        F: for<'a> FnMut(&'a StructuralValue, DefRegionKind) -> O,
        O: IntoMapResult,
    {
    }
...
impl<'a> InplaceValue<'a> {
    ...
    fn from_raw(raw: &'a mut TVMFFIAny) -> Self {
        Self {
            value: StructuralValue::from_raw(*raw),
            _scope: PhantomData,
        }
    }
...
impl StructuralVarRemap {
    ...
    pub fn get(&self, var: &StructuralValue) -> Result<Option<Any>> {
    ...
    pub fn set(&mut self, var: &StructuralValue, mutated_value: &Any) -> Result<()> {
        let key = object_identity_key(var.raw())?;
        self.entries.insert(
            key,
            MemoEntry {
...
impl<D: MapDispatch> NativeMapper<D> {
    fn map_raw(
        ...
        if is_plain_inline(raw.type_index) {
            let value = StructuralValue::from_raw(raw);
    ...
    fn map_current_raw(
        ...
        match self.order {
            ...
            WalkOrder::PostOrder => {
                ...
                let value = StructuralValue::from_raw(mapped_raw);
...
impl<D: MapDispatch> MutationDriver for NativeMapper<D> {
    ...
    fn var_remap_get_raw(&mut self, raw: TVMFFIAny) -> Result<Option<Any>> {
        self.remap.get(&StructuralValue::from_raw(raw))
    }

    fn var_remap_set_raw(&mut self, raw: TVMFFIAny, replacement: &Any) -> Result<()> {
        self.remap.set(&StructuralValue::from_raw(raw), replacement)
    }
}
...
impl<U: StructuralMutator> MutationDriver for U {
    ...
    fn var_remap_get_raw(&mut self, raw: TVMFFIAny) -> Result<Option<Any>> {
        self.var_remap_get(&StructuralValue::from_raw(raw))
    }

    fn var_remap_set_raw(&mut self, raw: TVMFFIAny, replacement: &Any) -> Result<()> {
        self.var_remap_set(&StructuralValue::from_raw(raw), replacement)
    }
}
...
fn dispatch_user_raw<U: StructuralMutator>(
    ...
    let result = if permit == Permit::MaybeInPlace && object_is_unique(raw) {
        ...
        mutator.dispatch_mutate(&StructuralValue::from_raw(raw), def_region_kind)
    };
