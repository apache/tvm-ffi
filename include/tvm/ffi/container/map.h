...
class DenseMapObj : public MapObj {
 private:
  ...
  struct ItemType {
    ...
    explicit ItemType(const key_type& key, const mapped_type& value) : data(key, value) {}
  };
 ...
 private:
   ...
   */
  bool IsFull() const {
    return static_cast<double>(size_ + 1) > static_cast<double>(NumSlots()) * kMaxLoadFactor;
  }
  /*!
   * \brief Increment the pointer
   * \param index The pointer to be incremented
