...
def test_structural_map_pre_and_post_order(
    ...
    assert list(root) == [2, 3]
    assert list(mapped) == [2, 3]


def test_structural_map_shared_root_uses_non_inplace_path() -> None:
    root = tvm_ffi.Array([1])
...
def test_structural_map_nested_containers() -> None:
    ...
    assert mapped[1][3] == 14
    assert mapped[2][5] == 16
    assert mapped.same_as(root)
    assert not mapped[0].same_as(list_source)
    assert list(list_source) == [1, 2]
    assert not mapped[1].same_as(map_source)
    assert map_source[3] == 4
    assert not mapped[2].same_as(dict_source)
    assert dict_source[5] == 6
...
def test_structural_map_maps_values_and_skips_keys() -> None:
    ...
    assert key_mapped.same_as(key_source)
    assert key_mapped[1] == "value"
    assert 11 not in key_mapped
