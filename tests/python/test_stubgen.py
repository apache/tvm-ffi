...
@pytest.mark.parametrize(
    ("payload", "size", "mirror", "extra_use"),
    [
        # Non-object payloads keep the 16-byte Any-cell mirror. Scalars mirror
        # at the schema-erased width: the cell stores the widened value for
        # every declared C++ width.
        pytest.param(TypeSchema("int"), 16, "Optional<i64>", None, id="int"),
        pytest.param(TypeSchema("float"), 16, "Optional<f64>", None, id="float"),
        pytest.param(TypeSchema("bool"), 16, "Optional<bool>", None, id="bool"),
        pytest.param(
            TypeSchema("Device"), 16, "Optional<DLDevice>", "tvm_ffi::DLDevice", id="device"
        ),
        pytest.param(
            TypeSchema("dtype"), 16, "Optional<DLDataType>", "tvm_ffi::DLDataType", id="dtype"
        ),
        # `ObjectRef`-derived payloads are pointer-sized object optionals
        # (#701): the mirror is Rust's niche-optimized `Option<T>`.
        pytest.param(TypeSchema("str"), 8, "Option<String>", "tvm_ffi::String", id="str"),
        pytest.param(TypeSchema("bytes"), 8, "Option<Bytes>", "tvm_ffi::Bytes", id="bytes"),
        ...
        pytest.param(TypeSchema("Callable"), 8, "Option<Function>", "tvm_ffi::Function", id="fn"),
        pytest.param(TypeSchema("Tensor"), 8, "Option<Tensor>", "tvm_ffi::Tensor", id="tensor"),
        pytest.param(TypeSchema("Shape"), 8, "Option<Shape>", "tvm_ffi::Shape", id="shape"),
    ],
)
def test_rust_optional_field_mirror(
    payload: TypeSchema, size: int, mirror: str, extra_use: str | None
) -> None:
    ...
    # Only the Any-cell mirror needs the crate's `Optional`; the object mirror
    # is the prelude `Option`.
    assert (RustUse("tvm_ffi::Optional") in imports.items) == mirror.startswith("Optional<")
...
def test_rust_optional_field_builder_store_and_setter() -> None:
    ...
    # mirror type as-is (no extra Option<T> sugar beyond the mirror itself).
    ...
    # An object payload mirrors as `Option<T>`, so the builder wraps it again.
    schema = NamedTypeSchema("x", TypeSchema("Optional", (TypeSchema("str"),)))
    text, _ = _gen_rust_object(_optional_field_info([schema]))
    assert "    pub x: Option<String>," in text
    assert "    x: Option<Option<String>>," in text
    assert "pub fn x(mut self, x: Option<String>) -> Self {" in text
...
@pytest.mark.parametrize(
    ("payload", "size"),
    [
        # A non-object payload must be the 16-byte `TVMFFIAny` cell; an 8-byte
        # reflected scalar Optional is not a mirrorable layout.
        pytest.param(TypeSchema("int"), 8, id="scalar-8-not-cell"),
        # An object payload must be the pointer-sized object optional; a
        # 16-byte reflected `Optional<str>` is a stale pre-#701 layout.
        pytest.param(TypeSchema("str"), 16, id="objref-16-not-pointer"),
...
def test_rust_optional_any_field_unsupported() -> None:
    # C++ `Optional<Any>` stays Any-backed, but its element rendering
    # (`ObjectRef`) is not `OptionalCompatible`: no compilable mirror, loud skip.
    schema = NamedTypeSchema("x", TypeSchema("Optional", (TypeSchema("Any"),)), size=16)
    with pytest.raises(UnsupportedTypeError) as exc:
        _gen_rust_object(_optional_field_info([schema], has_init=False))
    assert exc.value.origin == "Optional"
...
def test_rust_optional_builder_defaults() -> None:
    ...
    # `nullopt`-defaulted fields are prefilled with their mirror's disengaged
    # state: `Optional::none()` for the Any cell, `None` for the object mirror.
    assert "opt_j: tvm_ffi::Optional::none()," in text
    assert "opt_s: None," in text
    assert "opt_p: None," in text
    # Defaulted fields are stored as the bare mirror (no builder Option wrap).
    assert "    opt_s: Option<String>," in text
    assert "    opt_p: Option<Point>," in text
