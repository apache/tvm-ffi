...
class TestReplace:
    ...
    def test_replace_readonly_field_escape_hatch(self) -> None:
        """__replace__ uses object.__setattr__ which bypasses frozen guards,
        acting as an escape hatch even for C++ readonly fields."""
        pair = tvm_ffi.testing.TestIntPair(3, 4)
        pair2 = pair.__replace__(a=10)  # ty: ignore[unresolved-attribute]
        assert pair2.a == 10  # ty: ignore[unresolved-attribute]
        assert pair2.b == 4  # ty: ignore[unresolved-attribute]
        assert pair.a == 3  # original unchanged  # ty: ignore[unresolved-attribute]
