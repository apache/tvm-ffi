...
TEST(Reflection, RichObjectPtrFieldAndMethod) {
  ...
  TObjectPtrHolder holder(nullptr, std::move(composite_base));

  reflection::FieldGetter getter("test.ObjectPtrHolder", "alias");
  ...
  Function identity = reflection::GetMethod("test.ObjectPtrHolder", "number_identity");
