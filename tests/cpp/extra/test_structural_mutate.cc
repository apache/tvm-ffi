 ...
 private:
  ...
  Expected<Any> MutateImpl(AnyView value, bool maybe_inplace) {
    ...
    if (const auto* var = value.as<TVarObj>()) {
      ...
      if (var->name == "error") {
        Expected<Any> result = Unexpected(Error("ValueError", "cannot mutate error variable", ""));
    ...
    Expected<Any> result =
        maybe_inplace ? DefaultMaybeInplaceMutateExpected(value) : DefaultMutateExpected(value);
    TVM_FFI_S_MUTATE_MAYBE_EARLY_RETURN_WITH_ERROR_CONTEXT(result, value);
    return result;
  }
...
TEST(StructuralMutator, MaybeInplaceMutateUsesOwnershipToSelectCustomHook) {
  // Shared FreeVars are routed through the non-in-place hook.
  CheckCustomFreeVarHookRemap(true, "n-mutated");

  // A uniquely owned FreeVar may use its explicit maybe-in-place hook.
  constexpr const char* kTypeKey = "testing.StructuralMutateHookedVar";
  Function init = reflection::GetMethod(kTypeKey, "__ffi_init__");
  reflection::FieldGetter get_name(kTypeKey, "name");
  reflection::FieldGetter get_dependency(kTypeKey, "dependency");
  ObjectRef dependency = init("n", nullptr).cast<ObjectRef>();
  const Object* dependency_addr = dependency.get();
  ObjectRef root = init("n", dependency).cast<ObjectRef>();
  dependency = ObjectRef();
  const Object* root_addr = root.get();
  TestStructuralMutator mutator;

  ObjectRef mapped = mutator->MaybeInplaceMutate(root).cast<ObjectRef>();
  ObjectRef mapped_dependency = get_dependency(mapped).cast<ObjectRef>();

  EXPECT_EQ(mapped.get(), root_addr);
  EXPECT_EQ(root.get(), root_addr);
  EXPECT_EQ(mapped_dependency.get(), dependency_addr);
  EXPECT_EQ(get_name(mapped).cast<String>(), "n-maybe-mutated");
  EXPECT_EQ(get_name(mapped_dependency).cast<String>(), "n-maybe-mutated");
  EXPECT_EQ(get_name(root).cast<String>(), "n-maybe-mutated");
}
...
TEST(StructuralMap, MaybeInplaceMutationRespectsCallbackOrder) {
  ...
  // A shared root is mapped without invoking its maybe-in-place hook.
  {
    Array<Any> root{Any(TVar("lhs"))};
    Array<Any> root_alias = root;  // NOLINT(performance-unnecessary-copy-initialization)
    const Object* root_addr = root.get();

    Array<Any> mapped = StructuralMap<WalkOrder::kPostOrder>(root, map_var).cast<Array<Any>>();

    EXPECT_NE(mapped.get(), root_addr);
    EXPECT_EQ(root.get(), root_alias.get());
    EXPECT_EQ(root[0].cast<TVar>()->name, "lhs");
    EXPECT_EQ(mapped[0].cast<TVar>()->name, "lhs-mapped");
  }

  // A post-order callback runs after the container's in-place hook and observes mapped children.
  {
    Array<Any> root{Any(TVar("lhs")), Any(TVar("rhs"))};
    const Object* root_addr = root.get();
    ...
    Array<Any> mapped =
        StructuralMap<WalkOrder::kPostOrder>(root, map_var, [&](Array<Any> array) -> Expected<Any> {
          ++callback_count;
          EXPECT_EQ(array[0].cast<TVar>()->name, "lhs-mapped");
          EXPECT_EQ(array[1].cast<TVar>()->name, "rhs-mapped");
...
TEST(StructuralMap, PreOrderSelectsMutationModeFromCallbackResult) {
  auto increment = [](int64_t value) -> Expected<Any> { return Any(value + 1); };

  // Returning the input retains the caller's permission to mutate it in place. The callback's
  // temporary owning result must be released before the in-place hook checks uniqueness.
  {
    Array<Any> root{Any(int64_t{1})};
    const Object* root_addr = root.get();
    int callback_count = 0;

    Array<Any> mapped = StructuralMap<WalkOrder::kPreOrder>(
                            root,
                            [&](Array<Any> array) -> Expected<Any> {
                              ++callback_count;
                              return Any(std::move(array));
                            },
                            increment)
  ...
  // A fresh callback replacement is uniquely owned and may itself be mutated in place.
  {
    Array<Any> root{Any(int64_t{1})};
    const Object* replacement_addr = nullptr;

    Array<Any> mapped = StructuralMap<WalkOrder::kPreOrder>(
                            root,
                            [&](const Array<Any>&) -> Expected<Any> {
                              Array<Any> replacement{Any(int64_t{10})};
                              replacement_addr = replacement.get();
                              return Any(std::move(replacement));
                            },
                            increment)
                            .cast<Array<Any>>();

    EXPECT_EQ(mapped.get(), replacement_addr);
    EXPECT_EQ(mapped[0].cast<int64_t>(), 11);
    EXPECT_EQ(root[0].cast<int64_t>(), 1);
  }

  // A shared callback replacement must take the non-in-place path.
  {
    Array<Any> root{Any(int64_t{1})};
    Array<Any> replacement{Any(int64_t{10})};
    Array<Any> replacement_alias =
        replacement;  // NOLINT(performance-unnecessary-copy-initialization)

    Array<Any> mapped =
        StructuralMap<WalkOrder::kPreOrder>(
            root, [&](const Array<Any>&) -> Expected<Any> { return Any(replacement); }, increment)
            .cast<Array<Any>>();

    EXPECT_NE(mapped.get(), replacement.get());
    EXPECT_EQ(mapped[0].cast<int64_t>(), 11);
    EXPECT_EQ(replacement[0].cast<int64_t>(), 10);
    EXPECT_EQ(replacement.get(), replacement_alias.get());
  }
}

TEST(StructuralMap, PreOrderInvokesCallbackOnceWithoutMaybeInplaceHook) {
  TPair root(TVar("lhs"), TVar("rhs"));
  int callback_count = 0;

  TPair mapped =
      StructuralMap<WalkOrder::kPreOrder>(root, [&](const TPairObj* pair) -> Expected<Any> {
        ++callback_count;
...
TEST(StructuralMap, MapsMapValuesAccordingToOwnership) {
  auto map_var = [](const TVarObj* var) -> Expected<Any> {
    return Any(TVar(var->name + "-mapped"));
  };
  ...
  // Map keys are structural anchors and are not passed to callbacks.
  {
    Map<Any, Any> source{{Any(int64_t{1}), Any(TVar("value"))}};
    const Object* source_addr = source.get();
    int callback_count = 0;
    ...
    EXPECT_EQ(mapped.get(), source_addr);
    EXPECT_EQ(mapped.at(Any(int64_t{1})).cast<TVar>()->name, "value");
  }

  // A shared map still returns its source when every value is unchanged.
  {
    Map<Any, Any> source{{Any(int64_t{1}), Any(TVar("value"))}};
  ...
  // The first changed value lazily creates a copy when the source map is shared.
  {
    ...
    EXPECT_EQ(mapped.at(Any(int64_t{1})).cast<TVar>()->name, "value-mapped");
  }
}

TEST(StructuralMap, MapsDictValuesWithoutMappingKeys) {
  ...
  EXPECT_EQ(callback_count, 1);
  EXPECT_EQ(mapped.get(), source_addr);
  EXPECT_EQ(mapped.at(Any(int64_t{3})).cast<int64_t>(), 14);
}
