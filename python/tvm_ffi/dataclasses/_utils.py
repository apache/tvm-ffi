...
def method_init(type_cls: type, type_info: TypeInfo) -> Callable[..., None]:  # noqa: PLR0915
    ...
    for type_method in type_info.methods:
        if type_method.name == "__init__":
            init_method = type_method.func
            break
    else:
        raise ValueError(f"Cannot find constructor method: `{type_info.type_key}.__init__`")
    ...
    def __init__(self: type, *args: Any, **kwargs: Any) -> None:
        ...
        try:
            ...
            del kwargs
            self.__init_handle_by_constructor__(init_method, *args)
