...
def _collect_all_fields(type_info: TypeInfo) -> list[Any]:
    """Walk the parent chain to collect all fields in parent-first order."""
    ti: TypeInfo | None = type_info
    chain: list[TypeInfo] = []
    while ti is not None:
        chain.append(ti)
...
def _c_class_getstate(self: Any) -> dict[str, Any]:
    """Pickle ``__getstate__`` for @c_class objects using field reflection."""
    type_info: TypeInfo | None = getattr(type(self), "__tvm_ffi_type_info__", None)
    if type_info is None:
        raise TypeError(f"Cannot pickle {type(self).__name__}: no field info")
    all_fields = _collect_all_fields(type_info)
    state: dict[str, Any] = {}
    for field in all_fields:
...
def _reconstruct_c_class(cls: type, state: dict[str, Any]) -> Any:
    """Reconstruct a @c_class object from pickled field state.

    Uses the low-level ``__ffi_init__`` (with the KWARGS calling
    convention) so that any user-defined ``__init__`` transformations
    are bypassed -- the field values are applied exactly as saved.
    """
    type_info: TypeInfo | None = getattr(cls, "__tvm_ffi_type_info__", None)
    if type_info is None:
        raise TypeError(f"Cannot unpickle {cls.__name__}: no field info")
    all_fields = _collect_all_fields(type_info)
    # Build the argument list using the KWARGS calling convention that
    # __ffi_init__ expects: KWARGS, key1, val1, key2, val2, ...
    ffi_args: list[Any] = [core.KWARGS]
    non_init_fields: list[tuple[str, Any]] = []
    for field in all_fields:
        if field.name not in state:
            continue
        if field.c_init:
            ffi_args.append(field.name)
            ffi_args.append(state[field.name])
        else:
            non_init_fields.append((field.name, state[field.name]))
    obj: Any = cls.__new__(cls)  # ty: ignore[no-matching-overload]
    obj.__ffi_init__(*ffi_args)
    for name, value in non_init_fields:
        try:
            setattr(obj, name, value)
        except (AttributeError, TypeError):
            pass  # frozen or incompatible field; skip silently
    return obj


def _c_class_reduce(self: Any) -> tuple[Any, ...]:
    """Pickle ``__reduce__`` for @c_class objects using field reflection."""
    return (_reconstruct_c_class, (type(self), self.__getstate__()))
...
def _install_dataclass_dunders(
    ...
    *,
    ...
    # Install field-reflection-based pickle support.  This overrides the
    # JSON-based pickling inherited from CObject, which requires the
    # ffi.ToJSONGraphString / ffi.FromJSONGraphString C++ functions to be
    # registered.  Field-reflection pickling works for any @c_class type
    # whose C++ __ffi_init__ uses the auto-init KWARGS calling convention
    # (i.e. was auto-generated by ObjectDef, not an explicit init<T...>()).
    type_info: TypeInfo | None = getattr(cls, "__tvm_ffi_type_info__", None)
    if type_info is not None and type_info.fields is not None:
        ffi_init_method = next((m for m in type_info.methods if m.name == "__ffi_init__"), None)
        if ffi_init_method is not None and ffi_init_method.metadata.get("auto_init", False):
            if "__getstate__" not in cls.__dict__:
                setattr(cls, "__getstate__", _c_class_getstate)
            if "__reduce__" not in cls.__dict__:
                setattr(cls, "__reduce__", _c_class_reduce)
