...
def load_lib_ctypes(package: str, base: str, mode: str) -> ctypes.CDLL:
    """Load the tvm_ffi shared library by searching likely paths."""
    lib_path: Path = _find_library_by_basename(package, base)
...
def _find_library_by_basename(package: str, base: str) -> Path:  # noqa: PLR0912
    ...
    if sys.platform.startswith("win32"):
        ...
        lib_dll_names = (f"lib{base}.dylib", f"lib{base}.so")
...
def _resolve_and_validate(
    ...
    """Filter out paths that cannot be resolved."""
    for path in paths:
        try:
