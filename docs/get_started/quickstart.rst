...
as a TVM FFI compatible symbol ``__tvm_ffi_add_one_cpu/cuda``. It also exports the
function's metadata as a symbol ``__tvm_ffi__metadata_add_one_cpu/cuda``,
enabling type checking, validation, and stub generation.
